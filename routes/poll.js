...
module.exports = function (wss) {
			...
			if (req.user) {
				...
				if (req.cookies.votedOn) {
					if (JSON.parse(req.cookies.votedOn).includes(req.params.id)) {
						...
						votedOnArray = JSON.parse(req.cookies.votedOn);
						votedOnArray.push(req.params.id);
						votedOnStr = JSON.stringify(votedOnArray);
						res.cookie('votedOn', votedOnStr, {expire : new Date() + 9999});
                         
					}
				} else {
					votedOnArray = [req.params.id];
					votedOnStr = JSON.stringify(votedOnArray);
					res.cookie('votedOn', votedOnStr, {expire : new Date() + 9999});
				}
			}
	...
	router.get('/poll/new', function (req, res) {
		if (req.user) {
			res.render('poll', {
				'user': req.user,
